...
tse %>%
  ...
  survival::coxph(my_surv(HTN_AGEDIFF, INCIDENT_HTN) ~
                    scale(shannon) + BL_AGE + MEN + BMI + PREVAL_DIAB + HFC +
                    PREVAL_HFAIL_STRICT + CURR_SMOKE, data = .) %>% 
  broom::tidy(exponentiate = TRUE, conf.int = TRUE) %>%
  round_numeric_columns() %>% 
...
df_cox_results %>%
  ...
  dplyr::mutate(qval_fdr = p.adjust(p.value, method = "BH"))%>%
  dplyr::arrange(qval_fdr) %>%
  #write.csv("species-full-HTN.csv")
  DT::datatable()
```
...
ancombc2_species <- ANCOMBC::ancombc2(data = tse_species_BC,
            fix_formula = "INCIDENT_HTN + BL_AGE + MEN + BMI + PREVAL_DIAB + HFC + PREVAL_HFAIL_STRICT + CURR_SMOKE",
                         assay.type = "counts",
                         p_adj_method = "fdr",
                         verbose = TRUE,
                         neg_lb = TRUE)
...
beta2 <- tse %>%  mia::mergeFeaturesByRank(rank = "Species") %>% 
    ...
    mia::runRDA(assay.type = "relabundance",
                formula = assay ~ BL_AGE + MEN + BMI + PREVAL_DIAB + HFC +
                  PREVAL_HFAIL_STRICT + CURR_SMOKE + INCIDENT_HTN,
                distance = "bray",
                na.action = na.exclude)
```
```{r}
