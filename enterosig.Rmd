...
tse07<- readRDS("data/tse_greengenes2_MGS.rds") %>%
   mia::transformAssay(assay.type = "counts", method = "relabundance")
```
```{r}
# Filtering
altExp(tse07, "GenusPrevalent") <- mia::agglomerateByPrevalence(tse07, rank="genus", assay.type="relabundance",
                                        detection = 1/100, prevalence=5/100)
# Filter out "Other" taxa
altExp(tse07, "GenusPrevalent") <- altExp(tse07, "GenusPrevalent")[!(rownames(altExp(tse07, "GenusPrevalent")) 
					%in% c("Other")),]
```
```{r}
# Fit nmf with 5 components
x <- t(assay(altExp(tse07, "GenusPrevalent"), "counts"))
set.seed(3221)
nmf5 <- nmf(x, 5)
...
W <- as.data.frame(W)
colData(tse07) <- cbind(colData(tse07), W)

# Add NMF loadings in the metadata
metadata(tse07)$NMF_loadings <- H
...
colData(tse07)$ES_primary <- apply(Wrel, 1, which.max)
```

```{r}
#Plotting
## H is genus loadings for each NMF component
H <- metadata(tse07)$NMF_loadings

## W is sample memberships across NMF components
W <- as.data.frame(colData(tse07)) %>% dplyr::select(starts_with("nmf"))
...
colnames(W) <- c(1:5)
colnames(W) <- rownames(H)
W$clusters <- tse07$ES_primary
```
```{r}
#NMF community types
# Add the MDS dimensions for plotting
altExp(tse07, "GenusPrevalent") <- runMDS(
    altExp(tse07, "GenusPrevalent"),
    assay.type = "counts",
    FUN = vegan::vegdist,
    method = "bray")

plotReducedDim(
    altExp(tse07, "GenusPrevalent"), "MDS", 
    point_size = 1, theme_size = 13) +
    geom_point(aes(color = colData(tse07)$ES_primary)) +
